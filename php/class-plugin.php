...
class Plugin extends Plugin_Base {
	...
	public function register_assets() {
		...
		if ( ! $sb_active && is_array( $propertyid ) && array() !== $propertyid ) {
			wp_register_script( "{$this->assets_prefix}-mu", "//platform-api.sharethis.com/js/sharethis.js#property={$propertyid[0]}&product={$first_prod}-buttons", null, null, false );
		}
	}
	...
	public function register_admin_assets() {
		...
		if ( ! wp_script_is( 'sharethis-reaction-buttons-mua', 'registered' ) ) {
			wp_register_script( "{$this->assets_prefix}-mua", '//platform-api.sharethis.com/js/sharethis.js?product=inline-reaction-buttons', null, null, false );
