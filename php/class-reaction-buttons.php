...
class Reaction_Buttons {
	...
	public function define_sharethis_menus() {
		...
		if ( empty( $GLOBALS['admin_page_hooks']['sharethis-general'] ) ) {
			...
			// Main sharethis menu.
			add_menu_page(
				__( 'Share Buttons by ShareThis', 'sharethis-reaction-buttons' ),
				__( 'ShareThis', 'sharethis-reaction-buttons' ),
				'manage_options',
			...
			$this->general_hook_suffix = add_submenu_page(
				$this->menu_slug . '-general',
				__( 'ShareThis General Settings', 'sharethis-reaction-buttons' ),
				__( 'General Settings', 'sharethis-reaction-buttons' ),
				'manage_options',
				$this->menu_slug . '-general',
		...
		// If the property ID is set then register the share this menu.
	...
	public function enqueue_admin_assets( $hook_suffix ) {
		...
		if ( $hook_suffix === $this->hook_suffix ) {
			...
			wp_add_inline_script( "{$this->plugin->assets_prefix}-admin", sprintf( 'ReactionButtons.boot( %s );',
	...
	public function general_settings() {
		...
		// Check if the general settings have been already registered.
		if ( isset( $wp_settings_sections['property_id_section'] ) ) {
			return;
		}
				...
				?>
					<div class="notice notice-success is-dismissible">
						<p>
							...
							printf( esc_html__( 'Successfully reset your reaction button position display options!', 'sharethis-reaction-buttons' ), esc_html( sanitize_text_field( wp_unslash( $_GET['reset'] ) ) ) ); // WPCS: CSRF ok. Input var ok.
							?>
						</p>
					</div>
			<?php
	...
	public function activation_inform_notice() {
		...
		$gen_url = '<a href="' . esc_url( admin_url( 'admin.php?page=sharethis-reaction-buttons&nft' ) ) . '">configuration</a>';
		...
		if ( 'sharethis_page_sharethis-reaction-buttons' === $screen->base && get_transient( 'st-reaction-connection' ) && ! isset( $_GET['nft'] ) ) { // WPCS: CSRF ok. input var ok.
			?>
			<div class="notice notice-success is-dismissible">
				<p>
					<?php
					// translators: The product type.
					printf( esc_html__( 'Congrats! You’ve activated Reaction Buttons. Sit tight, they’ll appear on your site in just a few minutes!', 'sharethis-reaction-buttons' ), esc_html( $product ) );
					?>
				</p>
			</div>
			<?php
	...
	public function set_reaction_button_config() {
		...
		$reactions = array_map( 'sanitize_text_field', wp_unslash( $_POST['config']['reactions'] ) ); // WPCS: input var ok.
		$first = isset( $_POST['first'] ) && 'upgrade' !== $_POST['first'] ? false : true; // Input var okay.
		$type = isset( $_POST['type'] ) ? true : false; // Input var okay.
		$button = isset( $_POST['button'] ) ? sanitize_text_field( wp_unslash( $_POST['button'] ) ) : ''; // WPCS: input var ok.
		$config = isset( $_POST['config'] ) ? $_POST['config'] : ''; // WPCS: input var ok. WPCS: sanitization ok. Can't sanitize initially.  Sanitizing once checking value.

		// If user doesn't have a sharethis account already.
		if ( ! $type ) {
			$newconfig['inline-reaction'] = $config;
			$config = $newconfig;
		} else {
		...
		if ( ! $first ) {
			...
			$current_config['inline-reaction'] = array_map( 'sanitize_text_field', wp_unslash( $_POST['config'] ) ); // WPCS: input var ok.
		...
		if ( $first && 'platform' !== $button ) {
			update_option( 'sharethis_first_product', 'inline-reaction' );
			update_option( 'sharethis_inline-reaction', 'true' );
		}
	}
}
